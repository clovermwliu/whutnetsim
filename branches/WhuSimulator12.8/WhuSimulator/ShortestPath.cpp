...
CShortestPath::CShortestPath(CNode *n):CRoute(n)
{
}

CShortestPath::CShortestPath(const CShortestPath &c):CRoute(c)
{
...
CShortestPath::Clone()
{
	return new CShortestPath(*this);
}

NodeId_t 
CShortestPath::GetNextRouteId(NodeId_t dst)
{
...
void 
CShortestPath::copy_entrys(vector<CRouteEntry> &from, 
...
void
CShortestPath::update_route_table(NodeId_t dst)//更新路由表，并返回到dst的下一跳
{
	dijkstra();
}

void 
CShortestPath::dijkstra()
{
	//重新计算路由表
	_route_table.clear();
	...
	active.insert(make_pair(0, pn->get_id()));
