...
//去除了RouteTable类后可以去除
//////////////////////////////////////////////////////////////////////////
//#include <list>//去除了RouteTable类后可以去除
#include <map>
#include <iostream>
...
//Doc:Class reach it.
/*
RouteEntry中定义了当前结点到下一跳的Interface指针和下一跳IP地址，路由方法相关类依赖于RoutingEntry。
*/
#ifndef WIN32
public:
...
class Routing {
/*
描述：Routing类是派生其它所有路由方法的虚基类，每个路由方法类继承Routing类并且定义路由过程
*/
...
public:
  // Routing types
	//////////////////////////////////////////////////////////////////////////
	//去除无线路由相关类中可以换成
	typedef enum { STATIC, MANUAL, NIXVECTOR} RType_t;//枚举类型，定义路由方法的类型
	//////////////////////////////////////////////////////////////////////////
	//typedef enum { STATIC, DYNAMIC, MANUAL, NIXVECTOR, NVR, DSR, EIGRP } RType_t;//枚举类型，定义路由方法的类型
public:
  //Doc:Method
  Routing();//描述：默认构造函数
  ...
  //Doc:Method
  virtual ~Routing();//描述：默认析构函数，其它路由方法必须定义自己的析构函数
...
public:
  ...
  //描述：将新的路由信息加入存储路由信息的数据结构中

	virtual void Delete( IPAddr_t, Count_t) {};
	//描述：删除目标IP地址在存储路由信息的数据结构中的信息
  ...
  //描述：查找目标IP在存储路由信息的数据结构中对应的RoutingEntry
  ...
  virtual void    DataRequest(Node* n, Packet* p, void* v) {}//描述：封装协议头，未实现
  virtual bool    DataIndication(Interface* i, Packet* p) { return true; }//描述：解析数据包，未实现，直接返回true
...
public:
  Node*  node;   //绑定该路由对象的结点指针                                // Node to which it's attached

private:
  static Routing* defaultRouting;  //默认路由方法，当前使用的路由方法           // Points to active protocol
};
