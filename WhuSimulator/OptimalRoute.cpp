...
COptimalRoute::COptimalRoute(CNode *n):CRoute(n)
{
}

COptimalRoute::COptimalRoute(const COptimalRoute &c):CRoute(c)
{

}

COptimalRoute::~COptimalRoute(void)
{
...
COptimalRoute::Clone()
{
	return new COptimalRoute(*this);
}

NodeId_t 
COptimalRoute::GetNextRouteId(NodeId_t dst)
{
...
void
COptimalRoute::update_route_table(NodeId_t dst)//更新路由表，并返回到dst的下一跳
{
	dijkstra_degree();
}

void 
COptimalRoute::copy_entrys(vector<CRouteEntry> &from, 
...
COptimalRoute::dijkstra_degree()
{
	...
	active.insert(make_pair(pn->get_degree(), pn->get_id()));
	...
	while(!active.empty())
	{
		...
		for (; it != this_node->get_neighbor_end(); ++it)
		{
			...
			double new_cost = this_cost * pApp->graph->get_node(nv_id)->get_degree();
