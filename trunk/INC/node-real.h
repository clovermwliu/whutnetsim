...
// 无线动画中的画图部件
typedef std::deque<QCanvasItem*>        CanvasItemDeq_t;
...
// QT窗口信息
  ...
  QCanvasItem* Display(QTWindow*);//确定点显示的形状、大小等，显示位置为点当前的位置
  QCanvasItem* Display(const QPoint&, QTWindow*);//确定点显示的形状、大小等，显示位置参数中提供的位置
  ...
  void Color(const QColor&);//指定本地结点的颜色
  bool HasColor();//检查本地结点是否已经分配颜色
  QColor& Color();//返回本地结点的颜色
...
#ifdef HAVE_QT
  bool CustomShapeHelper(QPixmap&);//处理一般图形
#endif
//数据成员
private:
