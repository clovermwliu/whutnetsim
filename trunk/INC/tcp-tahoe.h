...
// Implements the Tahoe version of TCP
...
// collapse cwnd to one segment, and reduce slowstart threshold
// to 1/2 the current cwnd.

#ifndef __tcp_tahoe_h__
#define __tcp_tahoe_h__

#include "tcp.h"
#include "node.h"

//Doc:ClassXRef
class TCPTahoe : public TCP {
  //Doc:Class Class {\tt TCPTahoe} defines the behavior of the {\em Tahoe}
  //Doc:Class variation of {\em TCP}.  The base class functions
  //Doc:Class {\tt DupAck} and {\tt ReTxTimeout} are overridden here, but
  //Doc:Class all other base class {\tt TCP} functions are used.

public:
  TCPTahoe();
  TCPTahoe(Node* n);
  TCPTahoe(const TCPTahoe&); // Copy constructor
  virtual ~TCPTahoe();
