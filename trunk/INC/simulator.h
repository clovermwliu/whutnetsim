...
public:
  ...
  //Doc:Method
  Simulator();
    //除非使用分布式仿真，否则Simulator的构造函数没有参数

  virtual ~Simulator();
public:   
  //Doc:Method
  virtual void Handle(Event*, Time_t);
    //Simulator类是Handler类的子类，因为Simulator的对象必须要处理事件，尤其是进度和停止事件
    //参数1：一个指向要处理事件的指针
    //参数2：当前仿真的时间
  ...
  //Doc:Method
  Scheduler* SetScheduler(const Scheduler&); //没有实现 //delete
  ...
  //Doc:Method
  virtual void Progress(Time_t); //打印调试进度信息
    //定义请求一条进度信息并在标准输出中打印出来，这个信息被GTNS提供进度方法，或者使用
    //Simulator中ProgressHook的方法
    //参数1：进度区间，这个进度方法只会在仿真时间下周期性调用，而不是计算机时间
  ...
  //Doc:Method
  void    PrintStats();              // 打印调试信息
    //打印一些仿真中的细节信息，通常在Run方法退出，仿真结束的时候调用

  //Doc:Method
  void CommonRunInit();              // Run()之前的公共代码
    //Simulator::Run 和 DistributedSimulator::Run() 的公共代码

  //Doc:Method
  virtual bool DistributedSim()  {return false; }  //delete
    //Doc:Desc   Query if distributed sim (overridden by DistributedSimulator)
    //Doc:Return Always false here.
  
  //Doc:Method
  virtual void Run();                 // 开始进度事件
    //启动仿真，Run方法必须在所有的拓扑和数据流数据已经被设定之后调用。Run方法将
    //启动进度事件，而且会一直继续执行直到Stop事件被处理，或者Halt方法被调用，或者
    //直到事件队列为空。

  //Doc:Method
  void    StopAt(Time_t t);          // 在将来的时间设定一个Halt事件
    //在一个特定时间设定一个Stop事件，这个事件可以使Run方法在这个特定时间退出，
    //即使事件队列里还有未处理的事件。
    //参数1：设定Stop事件的仿真时间。

  //Doc:Method
  void    Halt();                    // 停止进度并退出
    //立即执行Stop事件并会导致Run方法退出

   //Doc:Method
  void    Silent(bool s) { silent = s; }
    //设置静默模式的开启或关闭，当开启时，Simulator不会在标准输出中打印任何信息
    //参数1：如果需要静默模式则设为true

  //Doc:Method
  void    Name(const std::string&);
    //对这次仿真指定一个名字，被http网页interface调用来给simulation编号
    //参数1：仿真的名字

  //Doc:Method
  void    DeleteObject(Object*);     // 设定一个删除对象事件
    //在特定的仿真时间设定一个事件删除一个指定的对象。有时候，一个GTNS对象可能
    //指明它不再被需要，但是一个C++对象不能删除自己。这个方法可以让一个对象设定
    //对自己的删除。
    //参数1：一个指向将被删除的对象的指针

  //Doc:Method
  void    ProgressHook(ProgressHook_t); // 添加一个进度的回调
    //每当一个进展事件被处理的时候指定调用一个子程序。这个进展事件会周期性的被调度，
    //周期的区间由Progress方法所指定。ProgressHook方法必须是静态方法，必须有且只有一个
    //参数，返回值为空。当ProgressHook方法被调用时，被传递的参数是当前的仿真时间。
    //参数1：在任何一个Progress区间中被调用的静态函数。
