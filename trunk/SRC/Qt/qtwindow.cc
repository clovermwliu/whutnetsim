...
// Subclass the QCanvasView class to process mouse clicks
...
// Event Handler
void QTWindow::Handle(Event* e, Time_t t)
{
...
void QTWindow::ProcessEvents()
{
  ...
  if (app) app->processEvents(1); // Once more to clear quit events
  if (quit)
    {
      app->quit();
      delete app;
...
void QTWindow::UpdateTopology(bool b)
...
void QTWindow::AddBackgroundLines(const LocationVec_t& points)
{
  bool first = true;
...
void QTWindow::PlaybackMode(bool pb)
{
  if (pb)
...
void QTWindow::DrawP2P(Node* f, Node* t)
...
void QTWindow::RedrawP2P(InterfaceBasic* ifaceSrc, InterfaceBasic* ifaceDst)
  ...
  for (NNIfLVec_t::size_type i = 0; i < displayedLinks.size(); ++i)
    {
      ...
      if ((nn.ifaceDest == ifaceDst && nn.ifaceSource == ifaceSrc) ||
        ...
        { // Found it
          QCanvasPolygonalItem* cl = nn.canvasItem;
...
void QTWindow::AddChild(QTChildWindow* c)
{
...
void QTWindow::DeleteChildWindow(QTChildWindow* c)
{
  for (ChildWindowList_t::iterator i = childWindows.begin();
       i != childWindows.end(); ++i)
    {
...
#ifdef MOVED_TO_LINK
void QTWindow::DisplayAllPackets()
{
...
void QTWindow::DisplayAllQueues()
{
...
#ifdef MOVED_TO_LINK
void QTWindow::DisplayOnePacket(Node* s, Node* d,
                                ...
                                const QColor& c)
{
  ...
  QCanvasPolygon* cp = new QCanvasPolygon(canvas);
...
#ifdef MOVED_TO_WLAN
void   QTWindow::WirelessTxStart(Node* n, InterfaceWireless* iFace, Node* d,
                                 Meters_t range)
{
  if (!n->IsReal()) return;
...
void   QTWindow::WirelessTxEnd(Node* n, InterfaceWireless* iFace, Node* d,
                               Meters_t range)
{
  if (!n->IsReal()) return;
...
#endif
//v1.01
//void   QTWindow::BasebandTxStart(BlueNode* n, Meters_t range)
//{
//  if (!n->IsReal()) return;
//  DEBUG(1, (cout << "Anim Bluetooth Tx Start, range " << range
//          << " time " << Simulator::Now() << endl));
//  BasebandTxStart(n, range, 0);
//}
//
//void   QTWindow::BasebandTxEnd(BlueNode* n, Meters_t range)
//{
//  if (!n->IsReal()) return;
//  DEBUG(1, (cout << "Anim Bluetooth Tx End, range " << range
//          << " time " << Simulator::Now() << endl));
//  BasebandTxEnd(n, range, 0);
//}
  ...
  if (it)
    {
      ...
      QCanvasCircle* c = new QCanvasCircle(rp.x() - lp.x(), canvas);
...
// Private for animating the Bluetooth tx
//v1.01
//void   QTWindow::BasebandTxStart(BlueNode* n, Meters_t range, Count_t it)
//{
//  if (it)
//    {
//      // Add the circle to the canvas items list (if not zeroth iteration)
...
//      QCanvasCircle* c = new QCanvasCircle(rp.x() - lp.x(), canvas);
//      // Set the color, with reduced intensity farther  away
//  	  //int FreqColor = (int)((static_cast<NodeBlueImpl*>(n->pImpl)->pBaseband)->Freq());
//	  int red, green, blue;
//	  red = static_cast<NodeBlueImpl*>(n->pImpl)->GetRed();
//	  green = static_cast<NodeBlueImpl*>(n->pImpl)->GetGreen();
//	  blue = static_cast<NodeBlueImpl*>(n->pImpl)->GetBlue();
//      int nred   = red   + (int)((double)(255-red)   * (it-1)/10.0);
//      int ngreen = green + (int)((double)(255-green) * (it-1)/10.0);
//      int nblue  = blue  + (int)((double)(255-blue)  * (it-1)/10.0);
//      c->setPen(QColor(nred, ngreen, nblue));
...
//  if (it < 10)
//    {
//      QTEvent* ev = new QTEvent(QTEvent::BASEBAND_TX_START);
//      ev->bluesrc = n;
//      ev->dst = NULL;
//      ev->iFace = NULL;
//      ev->range = range;
...
//  if (it == 10)
//    {
//      QTEvent* ev = new QTEvent(QTEvent::BASEBAND_TX_END);
//      ev->bluesrc = n;
//      ev->dst = NULL;
//      ev->iFace = NULL;
//      ev->range = range;
//      ev->iter = 0;
//      Time_t transDelay = Time("100us");
//      // Schedule next update event
//      Scheduler::Schedule(ev, transDelay, this);
//    }
//}
//
//void   QTWindow::BasebandTxEnd(BlueNode* n, Meters_t range, Count_t it)
//{
//  if (it)
//    { // Remove the circles one-by-one to animate end of transmission
//      // Must be real node here
